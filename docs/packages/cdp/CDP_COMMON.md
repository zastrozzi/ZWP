...
- **Root Files:**  
  ...
  - [README.md]({{ site.repo_root_url }}/packages/cdp/common/README.md)
...
- **src/**  
  ...
  - **lib/**  
    ...
    - **+state/**  
      ...
      - **actions/** – Contains action definitions for shared state. For example, [utility-dock.actions.ts]({{ site.repo_root_url }}/packages/cdp/common/src/lib/+state/actions/utility-dock.actions.ts) defines actions to open, close, and toggle utility dock panels.
      - **effects/** – Implements side effects (e.g. [utility-dock.effects.ts]({{ site.repo_root_url }}/packages/cdp/common/src/lib/+state/effects/utility-dock.effects.ts)).
...
- **Setup:**  
  Defined in [cdp-common.module.ts]({{ site.repo_root_url }}/packages/cdp/common/src/lib/cdp-common.module.ts), it also provides state environment providers via its `forRoot()` method.
...
- **Identifiers:**  
  The file [identifiers.ts]({{ site.repo_root_url }}/packages/cdp/common/src/lib/+state/identifiers.ts) defines:
...
- **State Assembly:**  
  The shared state is constructed by combining reducers (exported in [state.ts]({{ site.repo_root_url }}/packages/cdp/common/src/lib/+state/state.ts)) and is re-exported for ease of use.

### Actions

The CDP Common library includes actions for managing the Utility Dock, found in [utility-dock.actions.ts]({{ site.repo_root_url }}/packages/cdp/common/src/lib/+state/actions/utility-dock.actions.ts):
...
### Effects
- **Purpose:**  
  Effects in [utility-dock.effects.ts]({{ site.repo_root_url }}/packages/cdp/common/src/lib/+state/effects/utility-dock.effects.ts) listen for utility dock actions and perform asynchronous tasks such as updating overlay displays.
- **Implementation:**  
...
- **Implementation:**  
  The utility dock reducer (exported via [reducers.ts]({{ site.repo_root_url }}/packages/cdp/common/src/lib/+state/reducers/reducers.ts)) uses NgRx’s `createReducer` and `on` to update state immutably.

### Selectors
- **Purpose:**  
  Selectors extract slices of the CDP Common state for the Utility Dock.
- **Implementation:**  
  Utility Dock selectors (exported in [selectors/index.ts]({{ site.repo_root_url }}/packages/cdp/common/src/lib/+state/selectors/selectors.ts)) provide:
...
- **Models:**  
  ...
  - Shared models (in [model/common]({{ site.repo_root_url }}/packages/cdp/common/src/lib/model/common/index.ts)) such as the structure for utility dock panels.
- **Enums:**  
  Located in the `model/enums` folder (e.g. [utility-dock.panel-type.ts]({{ site.repo_root_url }}/packages/cdp/common/src/lib/model/enums/utility-dock.panel-type.ts)), which define available panel types for the Utility Dock.
