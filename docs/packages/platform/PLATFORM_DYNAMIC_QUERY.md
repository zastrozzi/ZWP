...
- **Services & Configuration:** Provides Live and Mock API services for executing queries, along with configuration tokens (see [dynamic-query.api-config.ts](../packages/platform/dynamic-query/src/lib/config/dynamic-query.api-config.ts)) that control base URLs for local and remote endpoints.
...
- **Root Files:**  
  ...
  - [README.md](../packages/platform/dynamic-query/README.md) – Brief summary
...
- **src/**  
  ...
  - **lib/**  
    ...
    - **config/** – Configuration files and injection tokens, such as the API configuration ([dynamic-query.api-config.ts](../packages/platform/dynamic-query/src/lib/config/dynamic-query.api-config.ts)).
...
### Actions
- **Location:** [actions/](../packages/platform/dynamic-query/src/lib/+state/actions/)
- **Purpose:**  
...
- **Examples:**  
  - `createStructuredQuery`, `getStructuredQuery`, `listStructuredQueries` in [structured-query.actions.ts](../packages/platform/dynamic-query/src/lib/+state/actions/structured-query.actions.ts)
  - Actions for queryable schema columns and relationships.

### Effects
- **Location:** [effects/](../packages/platform/dynamic-query/src/lib/+state/effects/)
- **Purpose:**  
  Listen to dispatched actions and perform remote API calls. Effects also handle side effects such as updating remote state.
- **Examples:**  
  - [StructuredQueryEffects](../packages/platform/dynamic-query/src/lib/+state/effects/structured-query.effects.ts) listens for create, get, list, update, and delete actions.
  - [QueryableSchemaTableEffects](../packages/platform/dynamic-query/src/lib/+state/effects/queryable-schema.table.effects.ts) for handling table-related remote calls.
  - Effects use helper functions such as `createRemoteEffect`, `remoteStateUpdateRequest`, etc.

### Reducers
- **Location:** [reducers/](../packages/platform/dynamic-query/src/lib/+state/reducers/)
- **Purpose:**  
...
### Selectors
- **Location:** [selectors/](../packages/platform/dynamic-query/src/lib/+state/selectors/)
- **Purpose:**  
  Provide selectors to get specific parts of the query state (e.g., selected query, remote pagination info, filter state).
- **Examples:**  
  - [StructuredQuerySelectors](../packages/platform/dynamic-query/src/lib/+state/selectors/structured-query.selectors.ts)
  - Selectors for queryable schema columns and tables.

### Facades
- **Location:** [facades/](../packages/platform/dynamic-query/src/lib/+state/facades/)
- **Purpose:**  
  Abstract NgRx interactions. They provide convenient methods to dispatch actions and select state data.
- **Examples:**  
  - [StructuredQueryFacade](../packages/platform/dynamic-query/src/lib/+state/facades/structured-query.facade.ts) for handling structured queries.
  - [QueryableSchemaTableFacade](../packages/platform/dynamic-query/src/lib/+state/facades/queryable-schema.table.facade.ts) for table operations.
...
- **API Routes:**  
  Centralized in the `api-routes/` folder (e.g. [api-routes.ts](../packages/platform/dynamic-query/src/lib/api-routes/api-routes.ts)) where endpoints for filters, tables, relationships, and query operations are defined.

- **Configuration:**  
  The API configuration is defined in the `config/` folder, notably in [dynamic-query.api-config.ts](../packages/platform/dynamic-query/src/lib/config/dynamic-query.api-config.ts).  
  It exposes:
...
- **Structured Query Components:**  
  - [StructuredQueryListComponent](../packages/platform/dynamic-query/src/lib/components/structured-query/structured-query.list.component.ts) displays a paginated list of queries.
...
- **Examples:**  
  - [StructuredQueryLiveAPIService](../packages/platform/dynamic-query/src/lib/services/live/structured-query.live.api.service.ts) for creating, reading, updating, and deleting structured queries.
  - [QueryableSchemaLiveAPIService](../packages/platform/dynamic-query/src/lib/services/live/queryable-schema.live.api.service.ts) which extends the Auth service and implements methods such as `getTable`, `listTables`, `listColumns`, and `getRelationship`.
...
- **Examples:**  
  - [QueryableSchemaMockAPIService](../packages/platform/dynamic-query/src/lib/services/mock/queryable-schema.mock.api.service.ts) for schema endpoints that throw “not implemented” errors or simulate delays.
