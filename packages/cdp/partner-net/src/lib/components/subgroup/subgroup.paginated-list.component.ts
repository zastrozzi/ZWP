...
@Component({
    selector: 'cdp-partnernet-subgroup-paginated-list',
    ...
    <div fxLayout="column" fxFlexFill>
        ...
        <zwp-paginated-table fxFlex="1 1 calc(100% - 55px)"
            [columns]="columns" [data]="(paginatedFilteredSubgroups$ | async) || []" [selectable]="true" [actionable]="true"
            [pagination]="subgroupRemotePagination$ | async"
        ...
        >
            <ng-template #rowActions let-row>
                <div fxLayout="row" fxLayoutAlign="start center" fxLayoutGap="8px">
                    <zwp-md-icon-button
                        icon="open_in_full" label="Open Group Details"
                        [iconPadding]="5"
...
export class SubgroupPaginatedListComponent implements OnInit, OnDestroy {
    private subgroupFacade = inject(State.Facades.PartnerNetSubgroupFacade)

    paginatedFilteredSubgroups$ = this.subgroupFacade.paginatedFilteredSubgroups$
    subgroupRemotePagination$ = this.subgroupFacade.subgroupRemotePagination$

    sortDirection: SortDirection = 'asc'
    sortKey: keyof Model.SubgroupResponse = 'name'

    subgroupStatusPipeSignature: TransformEnumPipeSignature = { input: Model.PartnerStatus, output: Model.PartnerStatusLabel }

    columns: ColumnInterface<Model.SubgroupResponse>[] = [
        { displayName: 'Name', dataLabel: 'name', sortable: true },
        { displayName: 'Status', dataLabel: 'status', sortable: true, transformEnumPipe: this.subgroupStatusPipeSignature },
    ...
    filterDefinitions: FilterDefinition[] = [
        ...
        { displayName: 'Status', name: 'status', type: ZWPFilterChipInputType.ENUM, enumDefinition: this.subgroupStatusPipeSignature }
    ]

    // Lifecycle Hooks
    ngOnInit() {
        this.subgroupFacade.resetSubgroupFilters(false)
        this.subgroupFacade.listSubgroups()
    }

    ngOnDestroy() {
        this.subgroupFacade.resetPagination()
    }

    // Event Handlers
        onInspectorClicked(row: Model.SubgroupResponse) {
        ...
        onNavigationClicked(row: Model.SubgroupResponse) {
            // this.panelLayoutFacade.addRightPanel({
            //     id: `${row.id}-table-details`,
            //     label: `Table Details - ${row.displayName}`,
            //     icon: 'schema',
        ...
        onClipboardClicked(row: Model.SubgroupResponse) {
            // this.panelLayoutFacade.addRightPanel({
            //     id: `${row.id}-table-details`,
            //     label: `Table Details - ${row.displayName}`,
            //     icon: 'schema',
        ...
        onSortChanged(sort: Sort) {
            this.sortKey = sort.active as keyof Model.SubgroupResponse
            this.sortDirection = sort.direction
            this.subgroupFacade.listSubgroups(null, 'none', { 
        ...
        onFiltersChanged(filterEvent: FilterChipEvent) {
            const filterChange: Partial<Model.SubgroupFilters> = handleFilterChipEvent(filterEvent)
            this.subgroupFacade.updateSubgroupFilters(filterChange)
        }
