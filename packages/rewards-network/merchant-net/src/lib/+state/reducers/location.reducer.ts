...
export interface LocationFeatureState extends BaseRemoteFeatureState {
    ...
    filters: Model.Filters.LocationFilters
}

export const locationEntityAdapter: EntityAdapter<Model.LocationResponse> =
    createEntityAdapter<Model.LocationResponse>()
...
export const initialLocationFeatureState: LocationFeatureState = {
    ...
    filters: Model.Filters.initialLocationFilters
}

export const locationReducer = createReducer(
    initialLocationFeatureState,
    on(
        LocationLocalActions.resetLocationState,
        () => initialLocationFeatureState
    ),
    on(
        LocationLocalActions.initialiseLocationState,
        () => initialLocationFeatureState
    ),
    on(
        LocationLocalActions.updateLocationFilters, (state, { filters }) => ({
            ...state,
            filters: {
                ...state.filters,
                ...filters
    ...
    on(LocationLocalActions.resetLocationFilters, (state) => ({ ...state, filters: Model.Filters.initialLocationFilters })),
    ...
    on(remoteStateUpdateFailure(LocationRemoteActions.identifiers), (state, { error }) => remoteFailureState(state, error)),

    on(
        LocationRemoteActions.create.success,
        (state, { response }) => ({
            ...state,
    ...
    on(
        LocationRemoteActions.get.success,
        (state, { response }) => ({
            ...state,
            locations: locationEntityAdapter.setOne(response, state.locations),
    ...
    on(
        LocationRemoteActions.list.request,
        (state, { pagination }) => ({
            ...state,
            locationsRemotePagination: {
                ...state.locationsRemotePagination,
    ...
    on(
        LocationRemoteActions.list.success,
        (state, { response }) => ({
            ...state,
            locations: locationEntityAdapter.setAll(response.results, state.locations),
            locationsRemotePagination: {
                ...state.locationsRemotePagination,
    ...
    on(
        LocationRemoteActions.update.success,
        (state, { response }) => ({
            ...state,
    ...
    on(
        LocationRemoteActions.delete.success,
        (state, { response }) => ({
            ...state,
            locations: locationEntityAdapter.removeOne(response.locationId, state.locations),
            selectedLocationId: 
                state.selectedLocationId === response.locationId ? null : state.selectedLocationId
        })
    )
)