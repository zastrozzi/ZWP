...
export class ZWPThemingEffects implements OnInitEffects {
    ...
    setDarkModeOnInit$ = createEffect(() =>
        this.actions$.pipe(
            ofType(ThemingActions.setInitialThemeColors),
            withLatestFrom(
                this.store.pipe(select(ThemingSelectors.automaticDarkModeSwitching)),
            ...
            map((settings) => {
                ...
                return ThemingActions.setDarkMode({
                    isDarkMode: settings.automaticSwitching ? darkModeOn : settings.darkMode,
                })
            })
        )
    )
    ...
    setSafariThemeColorMetatagOnDarkMode$ = createEffect(
        () =>
            this.store.pipe(select(ThemingSelectors.darkMode)).pipe(
                ...
                tap((value) => {
                    const themeColorTag: MetaDefinition = {
                        name: 'theme-color',
                        content: value.darkMode ? value.safariDark : value.safariLight,
                    }
    ...
    setDarkMode$ = createEffect(() =>
        this.actions$.pipe(
            ...
            map((value) => {
                return ThemingActions.setManyThemeColors({
                    colorTheme: value.darkMode ? value.darkColors : value.lightColors,
                    storeDarkMode: false,
                    storeLightMode: false,
                })
            })
        )
    )
