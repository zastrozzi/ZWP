...
@Component({
    ...
    template: `
        <zwp-md-button
            ...
            [labelColor]="buttonLabelColor | zwpColorTheme"
        ...
        <mat-menu #featureNavMenu="matMenu">
            ...
            >
                <zwp-md-button
                    *ngFor="let child of routeChildren | async"
...
export class FeatureNavButtonComponent {
    ...
    @Input() buttonLabelColor = 'primary'
        ...
        this.routeChildren = this.router.events.pipe(
            ...
            map(() => {
                return (this.route.routeConfig?.children ?? [])
                    ...
                    .map((child) => ({
                        path: `./${child.path}`,
                        navTitle: child.data?.['featureNavTitle'] ?? '',
                        navIcon: child.data?.['featureNavIcon'] ?? 'atr',
                    }))
            })
        )
    }
