...
@ZWPDebuggableInjectable({ serviceName: 'PlatformEnduserLiveAPIService', options: { skipMethodDebugger: true } })
export class PlatformEnduserLiveAPIService extends PlatformAuth.AuthedAPIService implements EnduserAPIService {
    private config = inject(IDENTITY_API_CONFIG)
    private baseUrl = inject(IDENTITY_API_BASE_URL)
    private enduserFacade = inject(State.Facades.EnduserFacade)
    private enduserSessionFacade = inject(State.Facades.EnduserSessionFacade)
