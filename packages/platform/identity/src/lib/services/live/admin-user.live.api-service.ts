...
import { AuditEventResponse, createHTTPParams, HTTPMethod, ZWPDebuggableInjectable, Nullable, PaginatedQueryParams, PaginatedResponse, serialiseDateQueryFilter, serialiseEnumQueryFilter, serialisePaginatedQueryParams, serialiseStringQueryFilter, upsertHTTPParam, upsertHTTPParams, serialiseBooleanQueryFilter } from "@zwp/platform.common";
...
@ZWPDebuggableInjectable({ serviceName: 'PlatformAdminUserLiveAPIService', options: { skipMethodDebugger: true } })
export class PlatformAdminUserLiveAPIService extends PlatformAuth.AuthedAPIService implements AdminUserAPIService {

    private config = inject(IDENTITY_API_CONFIG)
    private baseUrl = inject(IDENTITY_API_BASE_URL)
    private adminUserFacade = inject(State.Facades.AdminUserFacade)
    private adminUserActivityFacade = inject(State.Facades.AdminUserActivityFacade)
    private adminUserCredentialFacade = inject(State.Facades.AdminUserCredentialFacade)
    private adminUserDeviceFacade = inject(State.Facades.AdminUserDeviceFacade)
    private adminUserEmailFacade = inject(State.Facades.AdminUserEmailFacade)
    private adminUserSessionFacade = inject(State.Facades.AdminUserSessionFacade)
    ...
    listAdminUserEmails(adminUserId: Nullable<string>, pagination: Nullable<Partial<PaginatedQueryParams<Model.AdminUserEmailResponse>>>): Observable<PaginatedResponse<Model.AdminUserEmailResponse>> {
        ...
        this.adminUserEmailFacade.adminUserEmailFilters$.pipe(
            ...
            map((filters) => {
                ...
                if (filters.isVerified) { params = upsertHTTPParams(params, serialiseBooleanQueryFilter('is_verified', filters.isVerified)) }
            })
        ).subscribe().unsubscribe()
        return this.http.authedRequestWithOptions({
            accessToken: this.getAccessToken(),
    ...
    listAdminUserCredentials(adminUserId: Nullable<string>, pagination: Nullable<Partial<PaginatedQueryParams<Model.AdminUserCredentialResponse>>>): Observable<PaginatedResponse<Model.AdminUserCredentialResponse>> {
        ...
        this.adminUserCredentialFacade.adminUserCredentialFilters$.pipe(
            ...
            map((filters) => {
                ...
                if (filters.isValid) { params = upsertHTTPParams(params, serialiseBooleanQueryFilter('is_valid', filters.isValid)) }
            })
        ).subscribe().unsubscribe()
        return this.http.authedRequestWithOptions({
            accessToken: this.getAccessToken(),
    ...
    listAdminUserSessions(adminUserId: string, pagination: Nullable<Partial<PaginatedQueryParams<Model.AdminUserSessionResponse>>>): Observable<PaginatedResponse<Model.AdminUserSessionResponse>> {
        ...
        this.adminUserSessionFacade.adminUserSessionFilters$.pipe(
            ...
            map((filters) => {
                ...
                if (filters.dbUpdatedAt) { params = upsertHTTPParams(params, serialiseDateQueryFilter('db_updated_at', filters.dbUpdatedAt)) }
                if (filters.dbDeletedAt) { params = upsertHTTPParams(params, serialiseDateQueryFilter('db_deleted_at', filters.dbDeletedAt)) }
                if (filters.isActive) { params = upsertHTTPParams(params, serialiseBooleanQueryFilter('is_active', filters.isActive)) }
            })
        ).subscribe().unsubscribe()
        return this.http.authedRequestWithOptions({
            accessToken: this.getAccessToken(),
