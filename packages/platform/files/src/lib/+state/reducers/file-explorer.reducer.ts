...
export const fileExplorerReducer = createReducer(
    ...
    on(FileExplorerActions.addFileExplorerItemToSelection, (state, { id }) => ({ ...state, selectedItemIds: arrayDistinctUpsert(state.selectedItemIds, id) })),
    on(FileExplorerActions.removeFileExplorerItemFromSelection, (state, { id }) => ({ ...state, selectedItemIds: arrayDistinctRemove(state.selectedItemIds, id) })),
    on(FileExplorerActions.addFileExplorerItemsToSelection, (state, { ids }) => ({ ...state, selectedItemIds: arrayDistinctUpsert(state.selectedItemIds, ...ids) })),
    on(FileExplorerActions.removeFileExplorerItemsFromSelection, (state, { ids }) => ({ ...state, selectedItemIds: arrayDistinctRemove(state.selectedItemIds, ...ids) })),
    ...
    on(FileExplorerActions.setFileExplorerItemDragPreview, (state, { isMulti, itemCount, icon, label }) => ({...state, dragPreviewIsMulti: isMulti, dragPreviewItemCount: itemCount, dragPreviewIcon: icon, dragPreviewLabel: label}))
)