...
const selectedItems = createSelector(
    ...
    (ids, dict) => ids.compactMap(id => dict[id])
)

const currentDirectory = createSelector(
    currentDirectoryId,
    FileDataSelectors.fileDataItemEntities,
    (id, dict) => isNull(id) ? null : dict[id] ?? null
)
...
const currentDirectoryWithChildren = createSelector(
    ...
    FileDataSelectors.fileDataItemEntities,
    (curr, childIdsDict, itemsDict) => isNull(curr) ? null : ({
        ...curr, 
...
const currentDirectoryChildrenWithChildCounts = createSelector(
    ...
    FileDataSelectors.fileDataItemChildIdsByParent,
    (curr, childIdsDict) => isNull(curr) || isNil(curr.childFileDataItems) ? [] : curr.childFileDataItems.compactMap(child => ({
        ...child, 
        childFileDataItemsCount: childIdsDict[child.id]?.length
    } as Model.FileDataItem))
)

const rootDirectories = createSelector(
    FileDataSelectors.allFileDataItems,
    (allFileDataItems) => allFileDataItems.filter(e => isUndefined(e) ? undefined : isUndefined(e.parentFileDataItemId ?? undefined))
)

const rootDirectoriesWithChildCounts = createSelector(
    rootDirectories,
    FileDataSelectors.fileDataItemChildIdsByParent,
    (root, childIdsDict) => root.compactMap((rootEntity) => ({
        ...rootEntity, 
...
const explorerAllFiles = createSelector(
    explorerAllChildren,
    all => all.filter(e => e.isDir === false)
)

const explorerAllDirectories = createSelector(
    explorerAllChildren,
    all => all.filter(e => e.isDir === true)
)
