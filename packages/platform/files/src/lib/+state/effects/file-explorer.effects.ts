...
export class ZWPFileExplorerEffects {
    ...
    selectCurrentDirectoryFromRouter$ = createEffect(() => this.actions$.pipe(
    ...
    deselectAllFileExplorerItemsOnCurrentDirectoryChange$ = createEffect(() => this.actions$.pipe(
        ofType(FileExplorerActions.selectCurrentDirectory),
        map(() => FileExplorerActions.deselectAllFileExplorerItems())
    ))

    deselectAllFileExplorerItemsOnUpdateParent$ = createEffect(() => this.actions$.pipe(
        ofType(FileDataActions.updateParent),
        map(() => FileExplorerActions.deselectAllFileExplorerItems())
    ))

    deselectAllFileExplorerItemsOnUpdateParents$ = createEffect(() => this.actions$.pipe(
        ofType(FileDataActions.updateParents),
        map(() => FileExplorerActions.deselectAllFileExplorerItems())
    ))

    handleFileExplorerItemSelection$ = createEffect(() => this.actions$.pipe(
        ofType(FileExplorerActions.handleFileExplorerItemSelection),
        map(action => action.id),
        withLatestFrom(this.fileExplorerFacade.selectedItemIds$, this.keyboardFacade.metaKeyActive$, this.keyboardFacade.shiftKeyActive$),
    ...
    handleFileExplorerDragStartDragPreview$ = createEffect(() => this.actions$.pipe(
        ...
        map(([action, selectedIds]) => {
            ...
            if (!alreadySelected) { selectionCount++ }
            ...
            return FileExplorerActions.setFileExplorerItemDragPreview({isMulti, itemCount: selectionCount, icon, label})
        })
    ))

    handleFileExplorerDragStartItemSelection$ = createEffect(() => this.actions$.pipe(
    ...
    handleFileExplorerDragStartPreview$ = createEffect(() => this.actions$.pipe(
        ofType(FileExplorerActions.handleFileExplorerItemDragStart),
        map(action => FileExplorerActions.handleFileExplorerItemDragStartPreview({id: action.id, isDir: action.isDir, name: action.name}))
    ))

    handleFileExplorerDragDropped$ = createEffect(() => this.actions$.pipe(
        ofType(FileExplorerActions.handleFileExplorerItemDragDropped),
        withLatestFrom(this.fileExplorerFacade.selectedItemIds$),
        map(([action, selectedIds]) => {
            const dropInSelection = selectedIds.includes(action.dropId)
            if (dropInSelection) { return FileDataActions.updateParentsFailure({ reason: 'Cannot move item into itself' }) }
            else {
                return FileDataActions.updateParents({ updates: selectedIds.map((id) => ({itemId: id, parentId: action.dropId}))})
            }
        })
    ))
}